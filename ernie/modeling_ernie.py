...
def _build_ln(n_in, name, lr=1.0):
    return nn.LayerNorm(
        ...
        bias_attr=P.ParamAttr(
            ...
            initializer=nn.initializer.Constant(0.), learning_rate=lr), )
...
class AttentionLayer(nn.Layer):
    def __init__(self, cfg, name=None, lr=1.0):
        ...
        self.q = _build_linear(d_model, d_model_q,
                               append_name(name, 'query_fc'), initializer, lr)
        self.k = _build_linear(d_model, d_model_q,
                               append_name(name, 'key_fc'), initializer, lr)
        self.v = _build_linear(d_model, d_model_v,
                               append_name(name, 'value_fc'), initializer, lr)
        self.o = _build_linear(d_model_v, d_model,
                               append_name(name, 'output_fc'), initializer, lr)
