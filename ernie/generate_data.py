...
def generate_pretrain():
    ...
    dataset=dataset_factory.GenPretrainedConceptDataset(cfg,start=partial_no*partial_passage,end=partial_no*partial_passage+partial_passage)
    sampler = DistributedBatchSampler(dataset, batch_size=1,shuffle=False)
    loader = paddle.io.DataLoader(dataset, batch_sampler=sampler,collate_fn=dataset._collate_fn_gen,num_workers=cpu_count()//_nranks)
    dist.init_parallel_env()
    os.makedirs('gen_data/',exist_ok=True)
    all_steps = len(loader)
    save_id = 0
